...
train=fread('path/act_train.csv') %>% as.data.frame()
test=fread('path/act_test.csv') %>% as.data.frame()
#people data frame
people=fread('path/people.csv') %>% as.data.frame()

#unnecessary duplicate to char_2
people$char_1<-NULL

#changing the name of the variable to avoid confusion
names(people)[2:length(names(people))]=paste0('people_',names(people)[2:length(names(people))])

#changing the logical variabe to numerical datatype
...
importance_matrix <- xgb.importance(dimnames(D)[[2]], model = m2)
importance_matrix
xgb.plot.importance(importance_matrix)

#[299]	train-auc:0.990049
# Predict
out <- predict(m2, dtest)
sub <- data.frame(activity_id = test_activity_id, outcome = out)
write.csv(sub, file = "/home/apadala/Documents/Kaggle/model_sub.csv", row.names = F)
